...
class HorNet(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
                 depths=[2, 3, 18, 2], base_dim=64, drop_path_rate=0.4,
    ...
    def forward_features(self, x):
        outs = []
        length = 3 if self.use_rq else 4
...
class pinlv(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.first= nn.Conv2d(3,32,kernel_size=3,stride=2,padding=1)
        self.second= nn.Conv2d(3,32,kernel_size=3,stride=4,padding=1)
        self.first_conv = nn.Conv2d(3,32,kernel_size=3,stride=1,padding=1)
        self.first_conv2 = Cov_block(32,32,32)
        self.second_conv = Cov_block(32,32,32)
        self.last1 = Cov_block(64,32,32)
        self.last2 = Cov_block(32,32,64)
    ...
    def forward(self,x):
        ...
        x1 = self.first_conv2(x1)
        
        x2 = x2 - F.interpolate(x3, scale_factor=2, mode="nearest")
        x2 = self.second_conv(x2) 

        x1 = torch.cat((x1,F.interpolate(x2, scale_factor=2, mode="nearest")),dim=1)

        return self.last1(x1),self.last2(x2)
...
class MIX(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(4):
            ...
            layer_name = f'norm{i_layer}'
            self.add_module(layer_name, layer)
        self.last = nn.Conv2d(in_channels=512,out_channels=256,kernel_size=1)
