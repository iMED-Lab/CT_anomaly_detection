...
def calc_auc(prob_arr, gt_arr, mask_arr=None):
    prob_vec, gt_vec = extract_mask(prob_arr, gt_arr, mask_arr=mask_arr)
    roc_auc = metrics.roc_auc_score(gt_vec, prob_vec)
    fpr, tpr, thresholds = roc_curve(gt_vec, prob_vec)
    J = tpr - fpr
...
def numeric_score(pred_arr, gt_arr, kernel_size=(1,1)):  # DCC & ROSE-2: kernel_size=(3, 3)
    ...
    * TN = True Negatives
    ...
    FP = np.float64(np.sum(np.logical_and(pred_arr == 1, dilated_gt_arr == 0)))
    FN = np.float64(np.sum(np.logical_and(pred_arr == 0, gt_arr == 1)))
    TP = np.float64(np.sum(np.logical_and(pred_arr == 1, dilated_gt_arr == 1)))
    TN = np.float64(np.sum(np.logical_and(pred_arr == 0, gt_arr == 0)))
