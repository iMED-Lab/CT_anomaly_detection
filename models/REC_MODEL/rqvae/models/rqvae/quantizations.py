...
class VQEmbedding(nn.Embedding):
    ...
    def compute_distances(self, inputs):
        ...
        # print(embed_dim,inputs_shape[-1])
...
class RQBottleneck(nn.Module):
    ...
    def contrastive_loss(self):
        key = F.normalize(self.codebooks[0].weight[:-1,:],p=2,dim=1)
        weight1 =  torch.mm(key,torch.transpose(key,0,1))*self.codebooks[0].weight[-1,-1]
        label = torch.arange(self.n_embed[0]).to(weight1.device)
        loss = F.cross_entropy(input=weight1,target=label)
        return loss

    def forward(self, x):
        # print(self.codebooks[0].weight[:-1,:].shape)
        ...
        quants_trunc = quants_trunc.permute(0,3,1,2).contiguous()

        # contrastive_loss = self.contrastive_loss()

        return quants_trunc, commitment_loss, self.codebooks[0].weight[:-1,:], codes
...
class RQBooK(nn.Module):
    ...
    def __init__(self,
        ...
        self.commitment_loss = commitment_loss
        # self.in_conv = nn.Conv2d(in_channels=512,out_channels=channel,kernel_size=1)
        # self.out_conv = nn.Conv2d(in_channels=channel,out_channels=512,kernel_size=1)
    ...
    def forward(self, x,mask_quant_list = None):
        # x = self.in_conv(x)
        x = x.permute(0, 2, 3, 1).contiguous()
        x_reshaped = self.to_code_shape(x)
        quant_list, codes = self.quantize(x_reshaped)
        
        if mask_quant_list is not None:
            commitment_loss = self.compute_commitment_loss(x_reshaped, mask_quant_list)
        else:
            commitment_loss = self.compute_commitment_loss(x_reshaped, quant_list)
        ...
        return quants_trunc, commitment_loss, quant_list, codes
